...
class RekapDetailController extends GetxController
    with StateMixin<List<ParkingTicket>> {
  final rekaped = true.obs;
  final rekap = Rx<RekapTicket?>(null);
  @override
  void onInit() {
    rekap.value = Get.arguments as RekapTicket;
    getData();
    super.onInit();
  }
  ...
  Future<void> getData({bool shouldClearState = false}) async {
    ...
    final result = ResponseApi(
      data: rekap.value?.parkingTickets,
      success: true,
    );
    if (result.success) {
